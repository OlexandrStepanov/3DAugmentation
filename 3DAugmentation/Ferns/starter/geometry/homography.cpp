...
homography::~homography()
{
	...
	if (B_n) cvReleaseMat(&B_n);
	if (X_n) cvReleaseMat(&X_n);
	delete [] data.ptr;
}
...
void homography::transform_point(int u, int v, int * up, int * vp)
{
    float k = float(1. / (cvmGet(2, 0) * u + cvmGet(2, 1) * v + cvmGet(2, 2)));
    
    *up = int(roundf(k * (cvmGet(0, 0) * u + cvmGet(0, 1) * v + cvmGet(0, 2))));
    *vp = int(roundf(k * (cvmGet(1, 0) * u + cvmGet(1, 1) * v + cvmGet(1, 2))));
}
