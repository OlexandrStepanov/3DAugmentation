...
planar_pattern_detector::planar_pattern_detector(void)
{
  ...
  H_estimator = new homography_estimator;
    ...
    ransac_iterations_number = 1500;
    debug_image = 0;
}

planar_pattern_detector::~planar_pattern_detector(void)
{
  if (debug_image) cvReleaseImage(&debug_image);
  ...
  if (pyramid != 0) delete pyramid;
  if (classifier != 0) delete classifier;
}
...
//    sprintf(image_filename, "%s.bmp", filename);
//    mcvSaveImage(image_filename, model_image);
...
bool planar_pattern_detector::load(ifstream & f)
{
  ...
  cout << "> [planar_pattern_detector::load] Recognition rate: " << mean_recognition_rate << endl;
    ...
    f >> modelWidth >> modelHeight;
//  load_managed_image_in_pakfile(f, &model_image);
//    char image_filename[1000];
//    sprintf(image_filename, "%s.detector_data.bmp", image_name);
//    model_image = mcvLoadImage(image_filename, 0);
...
//    Save only image width and height
    f << model_image->width << " " << model_image->height << endl;
...
bool planar_pattern_detector::detect(IplImage * input_image)
{
...
IplImage * planar_pattern_detector::create_image_of_matches(void)
{
  ...
  if (!debug_image || 
      (debug_image->width != width || debug_image->height != height)) {
      if (debug_image) cvReleaseImage(&debug_image);
      debug_image = cvCreateImage(cvSize(width, height), IPL_DEPTH_8U, 3);
  }
    
    IplImage * result = debug_image;
    cvZero(result);
...
//            ss << setprecision(3) << model_points[i].class_score;
//            cvPutText(result, ss.str().c_str(), cvPoint(model_points[i].potential_correspondent->fr_u(),
