...
int pyr_yape06::detect(fine_gaussian_pyramid * pyramid, keypoint * keypoints, int max_number_of_keypoints)
{
  ...
  for(int i = 0; i < pyramid->number_of_octaves; i++) {
    compute_Ds(pyramid->aztec_pyramid[pyramid->level_from_scale(i)]);
    compute_laplacian(pyramid->aztec_pyramid[pyramid->level_from_scale(i)]);
    add_local_extrema(pyramid, pyramid->aztec_pyramid[pyramid->level_from_scale(i)], i);
  }
...
{
    if (!laplacian || 
        (laplacian->width != smoothed_image->width || laplacian->height != smoothed_image->height)) {
        if (laplacian) cvReleaseImage(&laplacian);
        laplacian = cvCreateImage(cvSize(smoothed_image->width, smoothed_image->height),
                                  IPL_DEPTH_32S, 1);
    }
    cvSetZero(laplacian);
