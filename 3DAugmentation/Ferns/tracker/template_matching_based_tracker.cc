...
template_matching_based_tracker::template_matching_based_tracker(void)
{
    ...
    randomGenerator = new RNG(time(NULL));
    noiseDerivation = 20.0;
}
...
//    Delete also matrices off course
    cvReleaseMat(&U0);
    cvReleaseMat(&U);
    cvReleaseMat(&I0);
    cvReleaseMat(&DU);
...
{
  int d = rand() % amp;
  float a = float(rand() % 1440) * 3.14159 * 2.0 / 1440.0;
...
void template_matching_based_tracker::add_noise(CvMat * V)
{
    ...
    for( i = 0; i < V->rows; i++ )
    {
        for( j = 0; j < V->cols; j++ )
        {
            double value = cvmGet( V, i, j );
            value += randomGenerator->gaussian(noiseDerivation);
            if (value < 0.0) value = 0.01;
            if (value > 255.0) value = 255.01;
            cvmSet(V, i, j, value);
        }
    }
}
...
void template_matching_based_tracker::find_2d_points(IplImage * image, int bx, int by)
{
  ...
  mcvSaveImage("gradientTracker.bmp", gradient);
    
  const float stepx = float(u0[2] - u0[0] - 2 * bx) / (nx);
  const float stepy = float(u0[5] - u0[1] - 2 * by) / (ny);
  for(int j = 0; j < ny; j++)
    for(int i = 0; i < nx; i++)
      get_local_maximum(gradient,
			int(u0[0] + bx + ((float)i + 0.5) * stepx),
			int(u0[1] + by + ((float)j + 0.5) * stepy),
			int(stepx), int(stepy),
			m[2 * (j * nx + i)],
			m[2 * (j * nx + i) + 1]);
